	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.onchange = function() {
				resetTrade(currentInitiator, player[parseInt(this.value, 8)], true);
			};
...
function land(increasedRent) {
	...
	// Special handling for Go square (assuming it's at index 0 or 1)
    if (p.position === 0 ) {
        updateMergedWindow("GET, SET, AND GO", null, null);
    } 
    // Special handling for square 21
    else if (p.position === 21) {
        updateMergedWindow(s.name, null, null);
    }
    else {
        updateMergedWindow(s.name, s.imageUrl, null);
    }
	$("#landed").show();
...
function updateMergedWindow(content, imageUrl = null, videoUrl = null) {
    var mergedWindow = document.getElementById('merged-window');
    var mergedInfo = document.getElementById('merged-info');
    ...
    // If video is provided, create video element
    if (videoUrl) {
        ...
        videoElement.autoplay = true; // Auto start video
        videoElement.loop = true;
        contentDiv.appendChild(videoElement);
    } 
    // Fallback to image or text if no video
    else if (imageUrl) {
    ...
    // Append content to merged info
    mergedInfo.appendChild(contentDiv);
}
